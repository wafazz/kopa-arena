...
function buildTimeSlots() {
    ...
    fetch('{{ route("bookings.booked-slots") }}?facility_id=' + opt.value + '&booking_date=' + bookingDate)
        ...
        .then(function(booked) {
            // Filter out child rows - we only care about parent bookings for display
            ...
            var bookedStarts = parentBookings.map(function(b) {
                return b.start_time.substring(0, 5);
            });
            ...
            slots.forEach(function(slot) {
                ...
                // Find exact match booking at this start time
                ...
                if (isExact) {
                    if (exactBooking.type === 'match' && exactBooking.status === 'match_open') {
                        ...
                        // Fully booked (normal or match-full)
