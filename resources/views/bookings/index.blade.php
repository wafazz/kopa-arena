...
@push('styles')
<style>
/* ── Elegant Calendar Theme ── */
.fc {
    --fc-border-color: rgba(255,255,255,.06);
    --fc-page-bg-color: transparent;
    --fc-neutral-bg-color: rgba(255,255,255,.03);
    --fc-list-event-hover-bg-color: rgba(25,135,84,.1);
    --fc-today-bg-color: rgba(25,135,84,.08);
...
.fc .fc-button {
    background: rgba(255,255,255,.05); border: 1px solid rgba(255,255,255,.1); color: #a8afc2;
    font-size: .8rem; font-weight: 500; padding: 6px 14px; border-radius: 8px;
    transition: all .2s ease; text-transform: capitalize; letter-spacing: .2px;
}
...
.fc .fc-button-active, .fc .fc-button:active {
    background: linear-gradient(135deg, #198754, #20c077) !important;
...
.fc .fc-col-header-cell {
    background: rgba(255,255,255,.03); color: #7c839a; font-size: .75rem;
    font-weight: 600; text-transform: uppercase; letter-spacing: 1px; padding: 12px 0;
}
.fc .fc-col-header-cell-cushion { color: #7c839a; text-decoration: none; }
.fc .fc-daygrid-day-number {
    color: #8b92a8; font-size: .82rem; font-weight: 500; padding: 8px 10px;
    transition: color .2s;
}
.fc .fc-daygrid-day:hover .fc-daygrid-day-number { color: #d1d5e0; }
.fc .fc-daygrid-day.fc-day-today {
    background: rgba(25,135,84,.08) !important; border-radius: 0;
}
.fc .fc-daygrid-day.fc-day-today .fc-daygrid-day-number {
    color: #2dd489; font-weight: 700;
    background: rgba(25,135,84,.15); border-radius: 50%; width: 30px; height: 30px;
...
.fc .fc-event {
    border: 0; border-radius: 6px; padding: 3px 7px; font-size: .74rem; font-weight: 500;
    cursor: pointer; transition: all .2s ease; border-left: 3px solid rgba(255,255,255,.3);
...
/* ── Toggle Buttons ── */
.view-toggle .btn {
    padding: 8px 20px; font-size: .82rem; font-weight: 500; border-radius: 10px;
    transition: all .25s ease; letter-spacing: .2px;
}
.view-toggle .btn.active {
    background: linear-gradient(135deg, #198754, #20c077); border-color: transparent; color: #fff;
    box-shadow: 0 4px 15px rgba(25,135,84,.35);
}
.view-toggle .btn:not(.active) {
    background: rgba(255,255,255,.04); border: 1px solid rgba(255,255,255,.1); color: #8b92a8;
}
.view-toggle .btn:not(.active):hover { background: rgba(255,255,255,.08); color: #d1d5e0; }

/* ── Calendar Card ── */
.calendar-card {
    background: linear-gradient(145deg, rgba(30,34,53,.95), rgba(39,43,58,.9));
...
.cal-filter {
    ...
    color: #c4c9d8 !important; border-radius: 10px !important; padding: 8px 14px !important;
    font-size: .82rem; transition: all .2s ease; backdrop-filter: blur(4px);
}
...
.booking-detail-row {
    ...
    background: rgba(255,255,255,.03); border-radius: 10px; border: 1px solid rgba(255,255,255,.04);
}
.booking-detail-row .detail-icon {
            ...
            <div class="au-card-inner">
                <div class="d-flex flex-wrap align-items-center justify-content-between mb-4 gap-3">
                    <div class="d-flex gap-2 flex-wrap">
                        <select id="calBranchFilter" class="form-select form-select-sm cal-filter" style="width:auto;min-width:170px;">
                        ...
                        <select id="calFacilityFilter" class="form-select form-select-sm cal-filter" style="width:auto;min-width:200px;">
                            ...
                            @foreach(Facility::where('status','active')->with('branch')->get() as $fac)
                            <option value="{{ $fac->id }}" data-branch="{{ $fac->branch_id }}">{{ $fac->name }} ({{ $fac->branch->name ?? '' }})</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            ...
                            <form action="{{ route('bookings.cancel', $booking) }}" method="POST" class="d-inline" data-confirm-delete>
                                @csrf
                                <button class="btn btn-sm btn-outline-danger" title="Cancel"><i class="fas fa-ban"></i></button>
                            </form>
                            @endif
                            <a href="{{ route('bookings.edit', $booking) }}" class="btn btn-sm btn-outline-primary" title="Edit"><i class="fas fa-pencil-alt"></i></a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
...
(function() {
    ...
    var branchFilter = document.getElementById('calBranchFilter');
    var facilityFilter = document.getElementById('calFacilityFilter');
    var editUrl = "{{ route('bookings.edit', ':id') }}";
    ...
    branchFilter.addEventListener('change', function() {
        ...
        var opts = facilityFilter.querySelectorAll('option[data-branch]');
    ...
    function initCalendar() {
        calendar = new FullCalendar.Calendar(calEl, {
            ...
            events: function(info, successCallback, failureCallback) {
                ...
                fetch("{{ route('bookings.calendar-events') }}?" + params.toString())
