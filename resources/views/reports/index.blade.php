    ...
    <div class="col-lg-12">
        <div class="au-card m-b-30">
            <div class="au-card-inner">
                <form action="{{ route('reports.generate') }}" method="POST" class="row g-3 align-items-end" id="reportForm">
                    @csrf
                    <div class="col-md-2">
                    ...
                    <div class="col-md-2" id="facilityFilter">
                        <label class="form-label">Facility (optional)</label>
                        <select name="facility_id" class="form-select">
                            <option value="">All Facilities</option>
                            @foreach($facilities as $facility)
                            <option value="{{ $facility->id }}">{{ $facility->name }} ({{ $facility->branch->name }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-2" id="branchFilter" style="display:none;">
            ...
            <div class="table-responsive table--no-card m-b-40">
                <table class="table table-borderless table-striped table-earning">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr><td colspan="8" class="text-center text-muted py-4">No orders found for this period</td></tr>
                        @endforelse
                    </tbody>
                    @if($orders->count())
                    <tfoot>
...
document.getElementById('reportCategory').addEventListener('change', function() {
    ...
    document.getElementById('branchFilter').style.display = (val === 'ecommerce') ? '' : 'none';
});
// Trigger on load
